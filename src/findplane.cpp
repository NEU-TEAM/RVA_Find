  ...
  if (z_in > - gh_ + 0.5)// not ground
    {
          ...
          Utilities::ecExtraction(cloud_projected_surface,cluster_indices,th_ratio_,10,10000000);
          ...
          for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
              {
                  ...
                  if (cloud_hull->points.size() > 10 && area_hull > th_area_)//area_hull > th_area_
                      {
                      ...
                      {
                          cerr << "Surface area is smaller than " << th_area_ << ", ignored.\n" << endl;
                      }
              }
      }
  else if (z_in < - gh_)
...
void FindPlane::processEachInliers(std::vector<pcl::PointIndices> indices_in, pcl::PointCloud<PointT>::Ptr cloud_in,  bool iswrite, bool isShow)
{
  if (indices_in.empty())
    {
      PCL_ERROR ("processEachInliers: Region growing didn't get cloud cluster.\n");
    }
  else
    {
      unsigned int k = 0;
      for (std::vector<pcl::PointIndices>::const_iterator it = indices_in.begin (); it != indices_in.end (); ++it)
        {
              ...
              if (iswrite)
                {
                      std::string name;
                      Utilities::generateName(k, "", ".pcd", name);
                      Utilities::writeCloud(cloud_fit_part, name);
                }

              if (isShow)
                {
                      //visualizeThings(cloud_fit_part, false);
                }
...
void FindPlane::getCloudByConditions(pcl::PointCloud<PointT>::Ptr cloud_source, pcl::PointIndices::Ptr &inliers_plane, double thc, double thn)
{
  for (int i = 0; i < cloud_source->points.size(); ++i)
    {
      float curv = cloud_source->points[i].curvature;
      float nz = cloud_source->points[i].normal_z;

      /// THRESHOLD
      //if point curve fulfill this criterion, consider it from plane
      if (curv < thc && nz > thn)
        {
...
void FindPlane::calRegionGrowing(pcl::PointCloud<PointT>::Ptr cloud, pcl::PointCloud<pcl::Normal>::Ptr normals)
{
      ...
      processEachInliers(clusters, cloud, false, true);
}

// merge cloud which from same plane
...
void FindPlane::findPlaneInCloud(pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr &cloud_in)
{
    ...
    pcl::PointIndices::Ptr indices_plane_curv(new pcl::PointIndices);
    getCloudByConditions(cloud_out, indices_plane_curv, th_curv_, th_norm_);
    getCloudByInliers(cloud_out, cloud_fit_plane, indices_plane_curv, false, false);
    if (cloud_fit_plane->empty())
        {
            PCL_ERROR ("FindPlane: No input cloud.\n");
            return;
        }
